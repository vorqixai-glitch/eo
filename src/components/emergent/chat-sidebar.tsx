...
export function ChatSidebar() {
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-border/40 bg-sidebar/60 backdrop-blur-xl text-sidebar-foreground flex flex-col">
      <div className="flex items-center gap-2 px-4 py-4 border-b border-sidebar-border/40">
        <div className="h-7 w-7 rounded-md bg-primary/20 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.2)]">
          <Leaf className="h-4 w-4 text-primary animate-pulse" />
        </div>
        <div className="flex flex-col">
          <span className="font-bold tracking-tight text-sm text-foreground">MOSS</span>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2">
        ...
        {projects.map((p) => {
          ...
          return (
            <Collapsible
              key={p.id}
              open={!isCollapsed}
              onOpenChange={(o) => setCollapsed((c) => ({ ...c, [p.id]: !o }))}
            >
              ...
              <CollapsibleContent className="pl-3 space-y-0.5">
                {items.length === 0 && (
                  <p className="px-2 py-1 text-xs text-muted-foreground/70">No chats yet</p>
                )}
        ...
        {(ungrouped.length > 0 || projects.length === 0) && (
          <div>
            {projects.length > 0 && (
              <div className="px-2 py-1 text-xs font-medium text-muted-foreground">Chats</div>
            )}
      ...
      <div className="border-t border-sidebar-border/40 p-2 space-y-2">
        {/* Live 3D Theme Selector Bar */}
        <div className="px-2 py-1">
          <span className="text-[10px] uppercase font-mono tracking-wider text-muted-foreground">
            3D Live Theme
          </span>
          <div className="flex items-center justify-between gap-1 mt-1.5 p-1 bg-background/50 rounded-lg border border-border/30">
            {[
              {
                id: "emerald",
                label: "Moss",
              ...
              },
              {
                ...
                label: "Midnight",
                icon: Laptop,
                color: "text-neutral-400 hover:bg-neutral-500/10",
              },
              {
                id: "light",
                label: "Paper",
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.success(`Switched to ${th.label} theme`);
                  }}
                  title={th.label}
                  className={cn(
                    ...
                    th.color,
                    isSelected
                      ? "bg-background shadow-sm border border-border/80 scale-105 font-medium ring-1 ring-primary/20"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="w-full flex items-center gap-2 rounded-md px-2 py-2 hover:bg-sidebar-accent text-left transition-all hover:translate-x-0.5">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Chats inside this project will be kept and moved to the top level. This can't be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
