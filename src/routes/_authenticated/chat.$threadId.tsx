...
function ChatThreadPage() {
  ...
  return (
    <div className="flex flex-1 min-h-0 w-full" key={threadId}>
      <ChatView threadId={threadId} onOpenArtifact={setArtifactId} activeArtifactId={artifactId} />
      {artifactId && <ArtifactPane artifactId={artifactId} onClose={() => setArtifactId(null)} />}
    </div>
  );
}
